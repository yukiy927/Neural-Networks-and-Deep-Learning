...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        sample_var = (x-running_mean).var(axis=0)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
...
def batchnorm_backward(dout, cache):
    ...
    # ================================================================ #
    N,D = dout.shape
    x_norm, x_center, std, gamma = cache
    ...
    dstd = (dx_norm * x_center * -std**(-2)).sum(axis=0)
    dvar = dstd/2/std
    dc = 1/N * dvar
    dx_center += 2 * x_center * dc

    # sample mean u has two paths
    du = -dx_center.sum(axis=0)-2/N * dvar * x_center.sum(axis=0)

    # calculation dx in two paths
    dx = dx_center + (1/N)*du
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
