...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:
                self.params['W{}'.format(i + 1)] = weight_scale * np.random.randn(hidden_dims[len(hidden_dims)-1], num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.use_batchnorm:
                # aff -> bn -> relu
                af_out, afcc = affine_forward(out,
                                                  self.params['W{}'.format(i+1)],
                                                  self.params['b{}'.format(i+1)])
                bn_out, bncc = batchnorm_forward(af_out,
                                                     self.params['gamma{}'.format(i+1)],
                                                     self.params['beta{}'.format(i+1)],
                                                     self.bn_params[i])
                out, rlcc = relu_forward(bn_out)
                cc[i+1] = (afcc, bncc, rlcc)
            else:
                out, cc[i+1] = affine_relu_forward(out,
                                                  self.params['W{}'.format(i+1)],
                                                  self.params['b{}'.format(i+1)])
            if self.use_dropout:
                out, dpcc[i+1] = dropout_forward(out, self.dropout_param)
        # last layer
        scores, cc[self.num_layers] = affine_forward(out,
        ...
        # ================================================================ #
        reg_loss = 0
        norm = lambda x: np.sum(np.square(x))
        loss, dz = softmax_loss(scores, y)
        for i in range(self.num_layers):
            reg_loss += self.reg * norm(self.params['W{}'.format(i + 1)])
        loss = loss + 0.5 * reg_loss

        # grads
        for i in range(self.num_layers,0,-1):
            # from the last layer to start
            if i == self.num_layers:
                d_out, grads['W{}'.format(i)], grads['b{}'.format(i)] = affine_backward(dz, cc[i])
            else:
                if self.use_dropout:
                    d_out = dropout_backward(d_out, dpcc[i])
                if self.use_batchnorm:
                    afcc, bncc, rlcc = cc[i]
                    dbn = relu_backward(d_out, rlcc)
                    daf, grads['gamma{}'.format(i)], grads['beta{}'.format(i)] = batchnorm_backward(dbn, bncc)
                    d_out, grads['W{}'.format(i)], grads['b{}'.format(i)] = affine_backward(daf, afcc)
                else:
                    d_out, grads['W{}'.format(i)], grads['b{}'.format(i)] = affine_relu_backward(d_out,
                                                                                                 cc[i])
