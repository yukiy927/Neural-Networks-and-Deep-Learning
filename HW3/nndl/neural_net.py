...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # ================================================================ #
    #f = lambda x, w, b: np.dot(x, w.T) + b
    h = np.maximum([0], X @ W1.T + b1)  # dim(h) =(5,10)
    scores = h @ W2.T + b2
    num_examples, num_classes = scores.shape
    #print("scores num of examples %d and classes %d" % (num_examples, num_classes))
    pass
    ...
    # overflow
    a = scores - np.max(scores, axis=1, keepdims=True)
    exp_a = np.exp(a)
    exp_sum = np.sum(exp_a, axis=1)
    correct_class = a[range(num_examples), y]
    loss = np.sum(np.log(exp_sum)) - np.sum(correct_class)
    exp_a = exp_a / exp_sum[:, np.newaxis]
    sfm_loss = loss / num_examples
    reg_loss = reg * (norm(W1) + norm(W2))
    loss = sfm_loss + 0.5 * reg_loss
    ...
    # ================================================================ #
    ind = np.zeros_like(exp_a)
    ind[np.arange(num_examples), y] = 1
    Q = exp_a - ind  # dim(Q) = 5*3
    # grads['W2']
    grads['W2'] = np.matmul(Q.T, h)
    grads['W2'] = grads['W2'] / num_examples + reg * W2  # dim =(3,10)
    # grads['b2']
    grads['b2'] = np.sum(Q, axis=0)
    grads['b2'] /= num_examples   # dim = (3, )
    # grads['W1']
    grads['W1'] = (W2.T @ Q.T * (h > 0).T) @ X
    grads['W1'] = grads['W1'] / num_examples + reg * W1
    # grads['b1']
    grads['b1'] = np.sum(W2.T @ Q.T * (h > 0).T, axis=1)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in np.arange(num_iters):
      ...
      # ================================================================ #
      idxs_batch = np.random.choice(num_train, X.shape[1])
